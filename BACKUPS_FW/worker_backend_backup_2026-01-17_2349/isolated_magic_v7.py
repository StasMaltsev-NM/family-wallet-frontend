...
<div id="magicSection" style="display:none; margin: 30px 10px; padding: 25px; border: 3px solid #FFD700; background: #000; border-radius: 30px; color: white; font-family: sans-serif;">
    ...
    <p style="font-size: 10px; opacity: 0.5; margin-bottom: 20px;">ДЕТЕКТОР КОНТРАКТА ВКЛЮЧЕН</p>
...
<script>
    ...
    async function applyMagicStyle(style) {
        ...
        try {
            ...
            console.log('MAGIC status:', res.status, 'ct:', ct);

            if (!res.ok || !ct.startsWith('image/')) {
                const txt = await res.text();
                console.error('MAGIC error body:', txt);
                alert('Ошибка магии: ' + (res.status === 403 ? 'Доступ запрещен' : 'ИИ занят'));
                return;
            }

            const blob = await res.blob();
            console.log('MAGIC blob size:', blob.size);
...
with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)
print("MAGIC_V7_WITH_DETECTOR_OK")
