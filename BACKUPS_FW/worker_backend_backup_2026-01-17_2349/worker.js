const VERSION = 'v2.2_delete_family_fixed_1767870069';
// ============================
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/api/auth/whoami' && request.method === 'GET') {
        ...
        // Проверяем родителя
        const parentAccess = await env.DB.prepare(
          'SELECT family_id FROM parent_access WHERE invite_code = ? AND role = "parent" AND revoked_at IS NULL'
        ).bind(inviteCode).first();

        if (parentAccess) {
          const family = await env.DB.prepare(
            'SELECT id, name, currency_name, currency_symbol FROM families WHERE id = ?'
          ).bind(parentAccess.family_id).first();
...
if (child) {
  // Считаем всего заработано
const lt = await env.DB.prepare(
  'SELECT COALESCE(SUM(reward_amount), 0) as total FROM tasks WHERE child_id = ? AND status = "CONFIRMED"'
).bind(child.id).first();  // ← ИСПРАВИЛИ!
  ...
  return json({
    ...
    lifetime_earnings: Number(lt?.total || 0),  // ← ДОБАВИЛИ!
    currency: {
      ...
      if (path === '/api/tasks/confirm' && request.method === 'POST') {
        ...
        if (action === 'confirm') {
          const newBalance = Number(child.balance) + Number(task.reward_amount);
          const newPending = Math.max(0, child.pending_balance - task.reward_amount);
      ...
      // GET /api/parents/codes
      if (path === '/api/parents/codes' && request.method === 'GET') {
        const inviteCode = request.headers.get('X-Invite-Code');
        if (!inviteCode) {
          return error('Требуется авторизация', 'NO_AUTH', 401);
        }
        ...
        if (!parentAccess) {
          return error('Доступ запрещён', 'FORBIDDEN', 403);
        }

        // Получаем все коды родителей для этой семьи
        const codes = await env.DB.prepare(
          'SELECT invite_code, label, created_at FROM parent_access WHERE family_id = ? AND role = "parent" AND revoked_at IS NULL ORDER BY created_at'
        ).bind(parentAccess.family_id).all();
      ...
      // POST /api/parents/invite/create
      if (path === '/api/parents/invite/create' && request.method === 'POST') {
        const inviteCode = request.headers.get('X-Invite-Code');
        if (!inviteCode) {
          return error('Требуется авторизация', 'NO_AUTH', 401);
        }
        ...
        // Получаем метку из body
        const body = await request.json();
        const label = body.label || 'Второй родитель';

        // Генерируем уникальный код
        const newInviteCode = 'FAM_' + Math.random().toString(36).substr(2, 6).toUpperCase();
        const accessId = 'parent_' + Date.now() + '_' + Math.random().toString(36).substr(2, 4);

        // Проверяем уникальность кода
        const existing = await env.DB.prepare(
          'SELECT id FROM parent_access WHERE invite_code = ?'
        ).bind(newInviteCode).first();

        if (existing) {
          return error('Ошибка генерации кода, попробуйте снова', 'CODE_COLLISION', 500);
        }

        // Сохраняем в parent_access
        await env.DB.prepare(
          'INSERT INTO parent_access (id, family_id, invite_code, role, label, created_at) VALUES (?, ?, ?, "parent", ?, datetime("now"))'
        ).bind(accessId, parentAccess.family_id, newInviteCode, label).run();

        return json({
          message: 'Код для второго родителя создан',
      ...
      // DELETE /api/children/delete — удаление ребёнка
      if (path === '/api/children/delete' && request.method === 'DELETE') {
        const inviteCode = request.headers.get('X-Invite-Code');
        if (!inviteCode) return error('Требуется авторизация', 'NO_AUTH', 401);
        ...
        if (!family) {
          return error('Только родители могут удалять детей', 'FORBIDDEN', 403);
        }
        ...
        if (!child_id) {
          return error('Отсутствует child_id', 'INVALID_INPUT');
        }
        ...
        if (!child) {
          return error('Ребёнок не найден или не принадлежит вашей семье', 'NOT_FOUND', 404);
        }


        // Отключить проверку FOREIGN KEY для каскадного удаления
        await env.DB.prepare('PRAGMA foreign_keys = OFF').run();

        await env.DB.batch([
          env.DB.prepare('DELETE FROM task_assignments WHERE child_id = ?').bind(child_id),
          env.DB.prepare('DELETE FROM purchases WHERE child_id = ?').bind(child_id),
          env.DB.prepare('DELETE FROM ledger_transactions WHERE child_id = ?').bind(child_id),
          env.DB.prepare('DELETE FROM reward_orders WHERE child_id = ?').bind(child_id),
          env.DB.prepare('DELETE FROM task_ledger WHERE child_id = ?').bind(child_id),
          env.DB.prepare('DELETE FROM events WHERE child_id = ?').bind(child_id),
          env.DB.prepare('DELETE FROM ai_profiles WHERE child_id = ?').bind(child_id),
          env.DB.prepare('DELETE FROM tasks WHERE child_id = ?').bind(child_id),
          env.DB.prepare('DELETE FROM reward_purchases WHERE child_id = ?').bind(child_id),
          env.DB.prepare('DELETE FROM dreams WHERE child_id = ?').bind(child_id),
          env.DB.prepare('DELETE FROM children WHERE id = ?').bind(child_id)
        ]);

        // Включить проверку FOREIGN KEY обратно
        await env.DB.prepare('PRAGMA foreign_keys = ON').run();

        return json({ message: 'Ребёнок удалён', child_id });
      }


      // DELETE /api/families/delete — удаление семьи и всех связанных данных
      if (path === '/api/families/delete' && request.method === 'DELETE') {
        const inviteCode = request.headers.get('X-Invite-Code');
        if (!inviteCode) return error('Требуется авторизация', 'NO_AUTH', 401);
        ...
        if (!family) {
          return error('Семья не найдена', 'NOT_FOUND', 404);
        }

        // Получить всех детей семьи
        ...
        // Отключить проверку FOREIGN KEY
        await env.DB.prepare('PRAGMA foreign_keys = OFF').run();

        // Удалить данные всех детей
        for (const childId of childIds) {
          await env.DB.batch([
            env.DB.prepare('DELETE FROM task_assignments WHERE child_id = ?').bind(childId),
            env.DB.prepare('DELETE FROM purchases WHERE child_id = ?').bind(childId),
            env.DB.prepare('DELETE FROM ledger_transactions WHERE child_id = ?').bind(childId),
            env.DB.prepare('DELETE FROM reward_orders WHERE child_id = ?').bind(childId),
            env.DB.prepare('DELETE FROM task_ledger WHERE child_id = ?').bind(childId),
            env.DB.prepare('DELETE FROM events WHERE child_id = ?').bind(childId),
            env.DB.prepare('DELETE FROM ai_profiles WHERE child_id = ?').bind(childId),
            env.DB.prepare('DELETE FROM tasks WHERE child_id = ?').bind(childId),
            env.DB.prepare('DELETE FROM reward_purchases WHERE child_id = ?').bind(childId),
            env.DB.prepare('DELETE FROM dreams WHERE child_id = ?').bind(childId),
            env.DB.prepare('DELETE FROM children WHERE id = ?').bind(childId)
          ]);
        }

        // Удалить данные семьи
        await env.DB.batch([
          env.DB.prepare('DELETE FROM reward_catalog WHERE family_id = ?').bind(family.id),
          env.DB.prepare('DELETE FROM rewards WHERE family_id = ?').bind(family.id),
          env.DB.prepare('DELETE FROM referrals WHERE referrer_family_id = ?').bind(family.id),
          env.DB.prepare('DELETE FROM families WHERE id = ?').bind(family.id)
        ]);

        // Включить проверку FOREIGN KEY обратно
        await env.DB.prepare('PRAGMA foreign_keys = ON').run();

        return json({ message: 'Семья и все связанные данные удалены', family_id: family.id });
      }
      ...
      if (path === '/api/rewards/list' && request.method === 'GET') {
        ...
        const rewards = await env.DB.prepare(`
          SELECT id, family_id, child_id, title, description, price, icon, is_permanent, created_at, updated_at
          FROM rewards
      ...
      if (path === '/api/rewards/create' && request.method === 'POST') {
        ...
        // Парсим тело запроса
        // 🔥 ЧИТАЕМ BODY КАК RAW TEXT (защита от двойного чтения)
        ...
        try {
          ...
          return error('Invalid JSON', 'PARSE_ERROR', 400);
        }
        
        console.log('🧩 PARSED BODY:', JSON.stringify(body, null, 2));
        
        const { title, description, price, icon, is_permanent, child_id } = body;

        // 🔥 ЖЁСТКАЯ ПРОВЕРКА child_id
        console.log('🔍 child_id:', child_id);
        console.log('🔍 child_id type:', typeof child_id);
        
        if (!child_id || child_id === 'undefined' || typeof child_id !== 'string') {
          console.log('❌ CRITICAL: child_id is invalid!', {
            child_id,
            type: typeof child_id,
            body
          });
          return error('child_id обязателен и должен быть строкой', 'MISSING_CHILD_ID', 400);
        }
        ...
        // Создаём награду
        await env.DB.prepare(`
          INSERT INTO rewards (id, family_id, child_id, title, description, price, icon, is_permanent, is_active, created_at, updated_at)
        ...
        return json({
          ...
          debug: {
            child_id_received: child_id,
            child_id_type: typeof child_id,
            child_id_length: child_id ? child_id.length : 0,
            bind_params: [rewardId, family.id, child_id, title]
          }
        });
      }
      ...
      if (path === '/api/rewards/purchase' && request.method === 'POST') {
        ...
        const newBalance = Number(child.balance) - Number(reward.price);
        await env.DB.prepare(
      ...
      if (path === '/api/rewards/confirm-received' && request.method === 'POST') {
        ...
        const purchase = await env.DB.prepare(
          'SELECT id FROM reward_purchases WHERE id = ? AND child_id = ? AND status IN (\'pending\', \'received\')'
      ...
      if (path === '/api/dreams/my' && request.method === 'GET') {
        ...
        // Обновляем current_amount на основе баланса
        const childBalance = await env.DB.prepare(
          "SELECT balance FROM children WHERE id = ?"
        ).bind(child.id).first();

        if (childBalance) {
          await env.DB.prepare(
            "UPDATE dreams SET current_amount = ? WHERE child_id = ? AND status = \"active\""
          ).bind(childBalance.balance, child.id).run();
        }
